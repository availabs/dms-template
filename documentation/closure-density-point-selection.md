...
8. **No request timeout on either density route.** Guaranteeing a fixed count with real spacing is
   inherently variable in cost per closure - the app-level timeout is removed entirely for
   `/trsp-memory-density-points` and `/trsp-memory-density` in `src/dms/packages/dms-server/src/index.js`,
   rather than tuned to another arbitrary number.
...
| Request timeout | none (removed) | `src/dms/packages/dms-server/src/index.js` |
