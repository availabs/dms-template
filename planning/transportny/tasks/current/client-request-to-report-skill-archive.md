# Client request → report: the skill arc — archive

**Project:** TransportNY

Read-only historical detail for the client-request-to-report skill work: the old-tool
template/archetype investigation, full implementation write-ups for the four original gaps plus
the follow-on wiring gaps, two real end-to-end client-request case studies, and test-cleanup
records. Current status, the intake posture/rules that are still actively used, and open items
live in [`client-request-to-report-skill.md`](./client-request-to-report-skill.md) — this file
exists only so the full investigation detail, code traces, and live-verification walkthroughs
aren't lost, without keeping the live doc long. Nothing here is current; where this archive and
the live doc disagree, the live doc wins.

Split out of the original single `client-request-to-report-skill.md` 2026-07-30 (which had grown
to ~1130 lines), verbatim except for this heading note, the removal of the top-level status
block/gap list (the live doc restates it in a condensed, de-duplicated form), and four small
wording fixes where a cross-reference ("above"/"below") pointed at content that moved to the live
doc (Storage decisions, Intake contract, Rules library) — each fix is noted inline where it occurs.
...
**Storage** (settles the "not yet chosen" sub-decision in the live doc's Storage decisions
table, diff-only per user pick): three new fields on the `reports_snap_2` row — `_spec`
(current full spec, `_`-prefixed working fields like `r._name`/`g._assigned` stripped before
persisting), `_specKeyMap` (spec `key` → section `trackingId`, the natural-key mapping
`--update` needs to find sections again), and `_specRevisions` (append-only `{at, note,
changed_paths}` log, capped at 200 entries — no full-snapshot-per-revision).
...
Landed as prose additions to `creating-reports.md` (not a new file — the storage decision
(live doc's Storage decisions table) already said rules live there) plus one small real
mechanism, since the intake contract's guess-and-flag posture needs to actually survive into
review, not just be a documented intention.

**What shipped:**
- **Intake checklist** — the posture table from the live doc's "Intake contract" section
  (infer / guess-and-flag / ask per input), plus the two corollaries earned by user correction
  (nothing is hard-required; road aliases don't resolve from `altrtename`).
- **Rules (earned corrections)** — the 8-rule list in the live doc's Rules library, moved into
  the skill file verbatim in spirit (geometry-not-period, same-season year-over-year, post-2017
  only, legend-ready route names, explicit difference anchors, don't gate on GIS heuristics,
