// Discover Hamilton County taxonomy: hazards (ul.main-menu li on Hazards page) + jurisdiction dropdown items.
const puppeteer = require('puppeteer-core');
const EDGE = 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe';
const BASE = process.env.MNY_BASE || 'https://hamilton.mitigateny.org';
...
(async () => {
  ...
  const juris = await page.evaluate(() => {
    const items = Array.from(document.querySelectorAll('li, [role="option"], [role="menuitem"], a, div, span'))
      ...
      .filter(t => /\((Town|Village|County|City)\)$/.test(t) && t.length < 45);
    return Array.from(new Set(items));
  });
  const countyBtn = await page.evaluate(() => { const b = Array.from(document.querySelectorAll('button')).find(x => /\((County|Town|Village)\)/.test(x.textContent||'')); return b ? b.textContent.trim() : null; });
