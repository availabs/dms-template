// Hamilton county-level scrape: landing, 5 top-nav sections, All-Hazards dashboard,
// 18 per-hazard Characteristics (with stabilization polling to beat the click->render lag),
// and the "Other Hazards" reference page. Single long-lived browser (county load is light).
const fs = require('fs');
...
(async () => {
  ...
  // Other Hazards reference page
  const oh = await clickHazard(page, 'Other Hazards');
  await sleep(2000);
  if (oh) save('county_other_hazards.txt', await grab(page));
  else log('  WARN: Other Hazards li not found');
