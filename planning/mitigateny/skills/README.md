...
Plans into it, applying QA-report findings back to it, and converting legacy 1.0 spreadsheet data into
the 2.0 workbook format.
...
| [`profiles/iem.md`](./profiles/iem.md) | The source plan is an **IEM** plan (Westchester 2026). The one consultant who **authors against the platform** — the document's heading tree *is* the 2.0 component order, so mapping is positional rather than interpretive. Read it for the two traps that mirroring creates: headings with **no prose beneath them** (14 of 36 annex headings were 0 characters), and heading spellings that **drift from the Card titles** (4 variants, 16 mis-attributions before aliases). |
...
| [`converting-capabilities-1-to-2/SKILL.md`](./converting-capabilities-1-to-2/SKILL.md) | Converting a legacy **1.0 Capabilities spreadsheet** into the 2.0 MNY workbook format, and reconciling it against an existing (often partial, uncleaned) 2.0 catalog/DMS export. Column mapping, geoid-crosswalk lookups for messy multi-jurisdiction and `"Countywide"` values, the Hazard Type and Capability Type rollups, and the merge rules. **Read its Stage 4 first**: the workbook can hold **two plausible Capability Type taxonomies** — the `Validations` sheet's 5-item FEMA list versus a ~16-item granular list meant for the Actions table — and picking the wrong one, or letting one column silently carry both, is the failure this skill exists to prevent. A **spreadsheet-side** skill: it produces a workbook, not DMS rows. Companion to the Hazards-of-Concern skill `converting-hoc-1-to-2`, which is installed as a managed skill rather than committed here. |
...
- [`westchester-baseplan-load-report.md`](./worked-examples/westchester-baseplan-load-report.md) —
  **a completed 2.0-pattern narrative load**: 177 Annotation slots, 334,761 ch, read-back verified,
  crosswalk driven to `GAP 0 / PARTIAL 0 / LOW 0`. Read it for the five transferable lessons — above
  all the **`{text:{root}}` vs `{root}`** shape trap, which returns success, sets every other field
  correctly, stores every character, and still renders an empty box. Also: why "nothing published"
  should be proved rather than asserted, and the lead-in rule that stops a merge heading orphaning
  the source after it.
...
| [`scripts/westchester/`](./scripts/westchester/) | The **IEM** pipeline, split into [`baseplan/`](./scripts/westchester/baseplan/) (county plan → the pattern's Annotation slots: `enumerate` → `build_inventory` → `docx_dump` → `section_map` → `build_crosswalk` → `build_fill_spec` → `backup_slots` → `fill_slots` → `verify_slots`) and [`annexes/`](./scripts/westchester/annexes/) (45 annexes → the Jurisdictions dataset). Two folders because **both contain a different `build_crosswalk.py`, `gen_report.py` and `fq.js`**. Has its own [README](./scripts/westchester/README.md) with run order, where the decisions live (`M`-list order *is* load order; `PARA_RULES`/`SENT_RULES`/`LEAD_INS`), and the canary-one-slot rule. `docx_dump.py` is the extractor to prefer — it walks `body.iterchildren()`, so no double-counted table paragraphs. |
...
| [`converting-capabilities-1-to-2/scripts/`](./converting-capabilities-1-to-2/scripts/) | **Not county-specific**, and unlike the folders above these are *patterns to adapt*, not run as-is: `tiered_type_assignment.py` (the tier-score + keyword-boost + guardrail engine that collapses candidate types into Primary/Secondary/Tertiary — re-parameterize its tier/keyword tables per taxonomy) and `build_workbook.py` (assembling the final multi-sheet `.xlsx`, since `.xlsb` can be read with `pyxlsb` but never written). The bundle's `references/gotchas.md` is the scan-through checklist version of the skill. |
