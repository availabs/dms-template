// Build the crosswalk CSV: annex-page rich-text box title -> jurisdictions dataset column.
// Authoritative source = the page's Card components (element-data.columns[] with show:true),
// intersected with the jurisdictions source's lexical columns.
// Inputs: juris_source.json, annex_components.json (from discover.mjs).
// Output: ../jurisdictional_annex_crosswalk.csv
...
for (const x of comps) {
  if (x.elementType !== 'Card' || !x.elementData) continue;
  let ed; try { ed = JSON.parse(x.elementData); } catch(e){ continue; }
  const shown = (ed.columns||[]).filter(c => c.show === true);
  for (const c of shown) {
    if (!(c.name in jcols)) continue;
    if (jcols[c.name].type !== 'lexical') continue;
...
// semantic tab grouping (annex UI area) for author orientation
const tabOf = {
...
// lexical columns with NO titled card on the annex page
const used = new Set(rows.map(r=>r.col));
for (const [name,meta] of Object.entries(jcols)) {
  if (meta.type !== 'lexical' || used.has(name)) continue;
