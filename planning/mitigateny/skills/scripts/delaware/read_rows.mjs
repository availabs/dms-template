// Read all Delaware jurisdiction rows, identify them, back up current content.
import fs from 'node:fs';
import { login, readRow, countyRowIds } from './annex_lib.mjs';
...
const ids = await countyRowIds(token);
console.error(`Delaware rows: ${ids.length}`);
const rows = [];
for (const id of ids) {
