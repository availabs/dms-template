...
def _in_textbox(node, stop):
    """True if `node` sits inside an anchored text box within `stop`.

    A run can carry a w:drawing whose w:txbxContent holds its own w:p paragraphs --
    pull-quotes, figure captions, callouts. Those are NOT part of the paragraph's text.
    """
    cur = node.getparent()
    while cur is not None and cur is not stop:
        if cur.tag in (qn('w:txbxContent'), qn('w:p')):
            return True
        cur = cur.getparent()
    return False
...
def _run_text(r_el):
    """
    Text of a w:r, including tabs and breaks, in document order -- but EXCLUDING anything
    inside an anchored text box.

    `r_el.iter()` descends through w:drawing into w:txbxContent, so a paragraph that anchors a
    pull-quote or figure caption picks that text up as if it were body text. On the Hagerty base
    plan that corrupted 5 paragraphs: block 24 gained a "Benefits of Mitigation Planning" pull
    quote ahead of its real sentence, and block 76 -- a Heading 4 reading "Social Media" -- came
    out as "Figure 3: Example Social Media Engagement" TWICE, losing the heading entirely.
    The doubling is `mc:AlternateContent`, which stores the same box under both a Choice and a
    Fallback branch.

    `docx_dump.py` uses `paragraph.text`, which never sees inside a text box. The two passes
    have to agree or the join they are validated on is meaningless.
    """
    out = []
    for c in r_el.iter():
        if _in_textbox(c, r_el):
            continue
        if c.tag == qn('w:t'):
