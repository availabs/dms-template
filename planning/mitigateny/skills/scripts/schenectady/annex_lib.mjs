// Shared helpers for the Schenectady jurisdictional-annex load.
// Auth: reads DMS_AUTH_TOKEN from the environment (mint it with the CLI — see
// src/dms/skills/authenticating-the-dms-cli.md). No credentials in this file. Reads: the jurisdictions dataset rows live in
// a split table and are NOT readable via byId — the only route that returns
// split-row data is the `edit` call's RETURNING, so we read with an empty merge
// (a no-op write that bumps updated_at + one changelog entry). Writes: go
// through the `dms dataset update` CLI command (edit call with the 4th `type`
// arg for split-table routing).
...
export const COUNTY_GEOID = '36093';
...
/** Enumerate the Schenectady jurisdiction row ids via the filtered `opts` route. */
export async function schenectadyRowIds(token) {
  ...
  const body = 'method=get&paths=' + encodeURIComponent(JSON.stringify([[
    'dms', 'data', `${APP}+${DATA_TYPE}`, 'opts', [optKey], 'byIndex', { from: 0, to: 49 },
  ]]));
  const r = await fetch(HOST + '/graph', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded', Authorization: 'Bearer ' + token },
