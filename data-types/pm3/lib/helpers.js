 ...
 * "resync" this file with ../../map21/helpers.js — divergence is the point.
...
async function createDataTable({ db, table_schema, table_name, columns = false }) {
  console.log(`[pm3] creating data table: ${table_schema}.${table_name}`);
...
/**
 * R4 — how many 15-minute bins a REPORTING_BINS entry could contain in a given year, if the feed
 * delivered every epoch. The denominator for pct_bins_reporting.
 *
 * Derived from the bin definition rather than hard-coded, because each bin has its own hours and
 * day-of-week mask: AMP is 4 hours x weekdays, WE is 14 hours x weekend days, ALL is everything.
 *
 * The day-of-week convention is the one the data query itself uses -- `toDayOfWeek(date, 2)`,
 * verified against ClickHouse as **Sunday = 0 ... Saturday = 6**. So REPORTING_BINS' dow lists read
 * as: [1,2,3,4,5] = Mon-Fri, [0,6] = Sun+Sat. Read off the runner rather than assumed, because a
 * silent off-by-one here would misstate completeness for every measure.
 */
function getExpectedBinsForYear({ hours, dow, year, timeBinSize = 15 }) {
  const binsPerHour = Math.floor(60 / timeBinSize);
  const dowSet = new Set((dow || []).map(Number));
  let matchingDays = 0;
  // Walk the actual year so leap years and the real weekday distribution are exact. Using
  // 365/7 * |dow| would be off by up to a day per weekday, which on a 4-hour bin is 16 bins.
  const d = new Date(Date.UTC(year, 0, 1));
  while (d.getUTCFullYear() === year) {
    if (dowSet.has(d.getUTCDay())) matchingDays += 1; // getUTCDay(): Sunday = 0, same as mode 2
    d.setUTCDate(d.getUTCDate() + 1);
  }
  return (hours || []).length * binsPerHour * matchingDays;
}
