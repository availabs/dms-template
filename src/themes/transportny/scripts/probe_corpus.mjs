...
//
// Usage:
//   node src/themes/transportny/scripts/probe_corpus.mjs --list
//   node src/themes/transportny/scripts/probe_corpus.mjs --capture [--only key1,key2]
//   node src/themes/transportny/scripts/probe_corpus.mjs [--only key1,key2]
//
// Options:
...
const FIXTURES_DIR = path.join(REPO, 'src/themes/transportny/scripts/report_probe_fixtures');
const MANIFEST_PATH = path.join(FIXTURES_DIR, 'golden-corpus.json');
const BASELINE_DIR = path.join(FIXTURES_DIR, 'baselines');
const PROBE = path.join(REPO, 'src/themes/transportny/scripts/report_probe.mjs');
...
for (const entry of entries) {
  ...
  if (!existsSync(baselinePath)) {
    console.log(`  NO BASELINE — run: node src/themes/transportny/scripts/probe_corpus.mjs --capture --only ${entry.key}`);
    anyBlockerOrMajor = true;
    continue;
  }
