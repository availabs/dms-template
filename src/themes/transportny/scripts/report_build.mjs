 ...
 *
 *   node src/themes/transportny/scripts/report_build.mjs <spec.json> [--summary|--dry-run] [--publish]
 *
 * WHY THIS EXISTS
 * ---------------
...
// ── DMS content constants (mirrors src/themes/transportny/scripts/convert_old_reports.py) ──────────
...
const CONVERTER_SCRIPT = resolve(REPO, 'src/themes/transportny/scripts/convert_old_reports.py');
...
function usage() {
  console.error(`usage:
  node src/themes/transportny/scripts/report_build.mjs <spec.json> [--summary|--dry-run] [--publish]
  node src/themes/transportny/scripts/report_build.mjs <spec.json> --update <page> [--note "..."]
  node src/themes/transportny/scripts/report_build.mjs <spec.json> --replace [--publish]
  node src/themes/transportny/scripts/report_build.mjs --from-page <page> [--out <spec.json>]
  ...
  To check that the built page actually renders, run the probe against it:
    node src/themes/transportny/scripts/report_probe.mjs <slug>            (published pages)
    node src/themes/transportny/scripts/report_probe.mjs edit/<slug> --auth  (draft-only pages)
`);
}
...
// src/themes/transportny/scripts/report_probe_fixtures/golden-corpus.json tagged
// "display._measurePick.routeIds" needs re-verifying (`node
// src/themes/transportny/scripts/probe_corpus.mjs --only <key>` before/after) — see
...
console.log(`check it renders:  node src/themes/transportny/scripts/report_probe.mjs ${DO_PUBLISH ? slug : `edit/${slug} --auth`}`);
