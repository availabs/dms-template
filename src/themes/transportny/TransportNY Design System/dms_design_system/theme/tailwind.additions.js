// TransportNY · tailwind config additions
// v0.1 · 2026-05-22
//
// Merge into the consuming project's `tailwind.config.{js,ts}` under
// `theme.extend`. Colors here back the literal hex values referenced in
// `theme.js`; if you change a token here, update the corresponding hex
// in theme.js or — preferred — replace those hex strings with the
// Tailwind class (e.g. `bg-nys-blue`).
...
const transportnyTailwind = {
  theme: {
    extend: {
      ...
      fontFamily: {
        // Use the same name DMS textSettings references
      ...
      // Section padding tokens — pick one per page.
      // Refer to these in layoutGroup styles, e.g. `py-section-comfortable`.
      spacing: {
        "section-dense":       "24px",
      ...
      // Container max-widths (referenced via `max-w-[var(--name)]`
      // or by setting a custom `container` plugin config).
      maxWidth: {
        "narrow":    "480px",
        "prose":     "720px",
        "split":     "1024px",
      ...
      // Brand-specific shadow stack
      boxShadow: {
        // Card: 1px crisp + 1px soft, no tinted ambient
      ...
      borderRadius: {
        // Card 8px · Form 6px · Sidebar items 0
        card:  "8px",
        input: "6px",
      },
    },
