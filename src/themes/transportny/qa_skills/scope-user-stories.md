# QA skill — Scope user stories for pages in the Scoping stage

A TransportNY **site-management QA skill**. It makes a pass over every page in the control room's
`sitemgmt_pages` dataset that is in the **Scoping** stage, proposes a set of **user stories** for each
...
Pipeline: **Scoping → In Development → Dev Review → Client Review → Approved**. A page enters
**Scoping** when we want to define *what it should do* before building. This skill is the work of the
Scoping stage: turn the page's purpose into a concrete, reviewable backlog of user stories.

Story stages: **proposed** (AI/dev drafted) → **accepted** (human agrees it's in scope) →
**verified** (built + confirmed on the live page).
...
1. **List the pages awaiting scoping.**
   ...
   ```
   Prints every `sitemgmt_pages` row with `stage === "Scoping"` — `page_key`, `name`, `surface_label`,
...
5. **Hand off.** The proposed stories now render in the page-QA page's **Features & user stories** card.
   ...
   Scoping until its stories are accepted (stage transitions are a human/dev action, not this skill's).
...
- Stories source: `sitemgmt_stories` — `app=npmrdsv5`, source `2186440`, view `2186441`, split data type
  ...
  sort_order`.
- Pages source: `sitemgmt_pages` — view `2184890`; the Scoping filter is `data->>'stage' = 'Scoping'`.
