...
export function npmrdsMeasureMenu({ state, dwAPI, currentComponent, isEdit, canEditSection, siblingSections = [] }) {
    ...
    // NPMRDS-specific: only offer this on a "report" (a page built from the
    // Report Page template — ReportRouteList + one-or-more AVL Graph
    // sections), never on an arbitrary page that happens to have an AVL
    // Graph section for some unrelated purpose. `element-type` is a plain
    // field on each raw section row, no JSON.parse needed.
    const isReportPage = siblingSections.some(s => s?.element?.['element-type'] === 'ReportRouteList');
    ...
    const applyPick = (partial) => {
        ...
        const composed = composeMeasureConfig({
            ...
            // the generic stub — see buildXAxisColumn. Safe because this
            // whole menu is isReportPage-gated: a report graph's Dataset IS
            // this source (see below), there's no other candidate it could be.
        ...
        dwAPI.setState(draft => {
            ...
            // Report-page wiring: this whole item-group only shows when
            // isReportPage is true (see cdn below), so every apply here
            // should leave the graph immediately assignable via
            // ReportRouteList — matching the Report Page template's own
            // pre-wired starter graph, not a bare "generate columns" tool.
            // Idempotent upsert: re-picking never duplicates the subscriber
            // entry or clobbers an unrelated one (e.g. hover_highlight).
    ...
    return [{
        ...
        cdn: () => isEdit && canEditSection && currentComponent?.useDataSource && isReportPage,
